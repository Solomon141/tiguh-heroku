...
router.post("/save", function (req, res) {
  const { Full_Name, RoleID, email, password } = req.body;
  ...
  con.query(sql, [email], function (error, result) {
    if (result.length >= 1) {
      ...
      con.query(
        "INSERT INTO `authuser`( `Full_Name`,`RoleID`,`email`, `password`, `vkey`) VALUES ( ?, ?, ?, ?, ?)",
        [Full_Name, RoleID, email, password, vkey],
        (error, result) => {
...
router.get("/users", function (req, res) {
  con.query("SELECT * FROM `authuser`", (error, result) => {
    if (error) return res.status(400).json(error);
    res.status(200).json(result);
  });
});

router.get("/usersWithRole", function (req, res) {
  con.query(
    "SELECT `authuser`.`id`, `email`, `Is_Active`, `Full_Name`, `phoneNum`, `Description` FROM `authuser` INNER JOIN `lookup` ON `lookup`.`ID` = `authuser`.`RoleID` WHERE `RoleID` <> 101",
    (error, result) => {
      if (error) return res.status(400).json(error);
      res.status(200).json(result);
    }
  ...
  if (email && password) {
    ...
    con.query(sql, [email, password], function (error, resdata) {
      if (resdata.length === 1) {
        ...
        let fullname = resdata[0].Full_Name;
        let RoleID = resdata[0].RoleID;
        // let RoleName = resdata[0].RoleID;
...
router.get("/verify/:vkey", function (req, res) {
  ...
  con.query(
    `UPDATE authuser SET is_active = '1', email_verified_at= NOW() WHERE vkey = '${vkey}' `,
    function (err, result) {
      if (err) {
        res.send({
          error: true,
